...
def profile(request):
    ...
    for achievement in all_achievements:
        ...
        if achievement.required_entries > 0:
            progress_percent = min(100, (progress.total_entries / achievement.required_entries) * 100)
        elif achievement.required_time_periods > 0:
            progress_percent = min(100, (progress.unique_time_periods / achievement.required_time_periods) * 100)
        elif achievement.required_streak > 0:
            progress_percent = min(100, (progress.longest_streak / achievement.required_streak) * 100)
        ...
        achievements_by_category[achievement.get_category_display()].append({
            ...
            'earned_at': next((ea.earned_at for ea in earned_achievements if ea.achievement_id == achievement.id), None)
        })
    ...
    most_visited_periods = DiaryEntry.objects.filter(user=user)\
        .values('time_period')\
    ...
    # Recent entries
    recent_entries = DiaryEntry.objects.filter(user=user)\
        .order_by('-date_of_journey')[:5]
    ...
    context = {
        ...
        'achievement_progress': round((earned_achievements.count() / all_achievements.count()) * 100) if all_achievements.count() > 0 else 0
    }
